...
export async function createNodeApplication({ quantity }) {
  const opId = await getMyOperatorId();
  if (!opId) throw new Error('로그인이 필요합니다.');
  const qty = Math.max(1, parseInt(quantity, 10) || 1);
  const { data, error } = await supabase.from('deposits').insert({
    operator_id: opId, quantity: qty, amount_usdt: qty * NODE_PRICE_USDT, network: 'TRC20', status: 'pending',
  }).select().single();
  if (error) throw error;
  return data;
}
...
export async function getMyDeposits() {
  const { data, error } = await supabase
    .from('deposits').select('id, quantity, amount_usdt, status, created_at').order('created_at', { ascending: false });
  if (error) throw error;
  return data || [];
}
