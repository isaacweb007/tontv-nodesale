...
export async function adminOperatorDetail(id) {
  const [{ data: op }, { data: down }, { data: nodes }, { data: stakes }, { data: bal }, { data: tx }] = await Promise.all([
    supabase.from('node_operators').select('id, display_name, email, phone, status, is_admin, is_center_head, kyc_status, referral_code, referred_by, created_at').eq('id', id).maybeSingle(),
