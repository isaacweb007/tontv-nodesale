...
export const TIERS = {
  ...
  plus:  { code: 'plus',  name: '플러스', en: 'Plus',  price: 3000,  nodes: 3,  bonus: 3 },
  pro:   { code: 'pro',   name: '프로',   en: 'Pro',   price: 5000,  nodes: 5,  bonus: 5 },
...
export async function adminOrgTree() {
  const { data, error } = await supabase.from('node_operators')
    .select('id, display_name, email, referred_by, referral_code, rank_id, created_at, nodes(count)')
...
/* ============================================================
   보상플랜 (직급 · 추천수당 · 보너스) — 설정 가능한 골격
   공개 read(active) / 관리자 write. 값은 어드민에서 입력.
   ============================================================ */

// 공개: 대시보드/노드페이지가 플랜을 읽음
...
// 본인 조직 요약 (SECURITY DEFINER 함수 — 하위 수·추천수당·직급)
export async function getMyReferralSummary() {
  const { data, error } = await supabase.rpc('my_referral_summary');
  if (error) throw error;
  return (data && data[0]) || { referral_code: '', downline_count: 0, referral_earned: 0, rank_name: null, rank_color: null };
}
...
// 관리자: 직급 CRUD
export async function adminListRanks() {
  const { data, error } = await supabase.from('ranks').select('*').order('sort_order', { ascending: true });
  if (error) throw error; return data || [];
}
export async function adminSaveRank(r) {
  const row = { code: r.code || null, name: r.name, sort_order: +r.sort_order || 0, min_nodes: +r.min_nodes || 0, min_referrals: +r.min_referrals || 0, min_group_nodes: +r.min_group_nodes || 0, bonus_pct: +r.bonus_pct || 0, benefits: r.benefits || '', color: r.color || '#e8112d', active: r.active !== false };
  const q = r.id ? supabase.from('ranks').update(row).eq('id', r.id) : supabase.from('ranks').insert(row);
  const { error } = await q; if (error) throw error; return true;
}
export async function adminDeleteRank(id) {
...
// 관리자: 회원(노드 구매자) 상세 — 조직(상위/하위)·노드·스테이킹·잔액
export async function adminOperatorDetail(id) {
  const [{ data: op }, { data: down }, { data: nodes }, { data: stakes }, { data: bal }, { data: tx }] = await Promise.all([
    supabase.from('node_operators').select('id, display_name, email, phone, status, is_admin, kyc_status, referral_code, referred_by, rank_id, created_at').eq('id', id).maybeSingle(),
    supabase.from('node_operators').select('id, display_name, email, created_at, referral_code').eq('referred_by', id).order('created_at', { ascending: false }),
    supabase.from('nodes').select('id, status, price_usdt, purchased_at').eq('operator_id', id),
    supabase.from('staking').select('id, amount, months, apy, status, ends_at').eq('operator_id', id),
    supabase.from('wallet_balances').select('xont_balance').eq('operator_id', id).maybeSingle(),
    supabase.from('xont_transactions').select('amount, direction, kind, created_at').eq('operator_id', id).order('created_at', { ascending: false }).limit(20),
  ]);
  let upline = null;
  if (op && op.referred_by) {
    const { data: up } = await supabase.from('node_operators').select('display_name, email, referral_code').eq('id', op.referred_by).maybeSingle();
    upline = up || null;
  }
  return { op, upline, downline: down || [], nodes: nodes || [], stakes: stakes || [], balance: bal ? Number(bal.xont_balance) : 0, tx: tx || [] };
}
export async function adminAssignRank(operatorId, rankId) {
  const { error } = await supabase.from('node_operators').update({ rank_id: rankId || null }).eq('id', operatorId);
  if (error) throw error; return true;
}
