...
// XONT · USDT · TONX 잔액
//   USDT: 영업수당 · 스테이킹 지급   ·   TONX: 노드 보상 (운영수익 분배)
export async function getWalletBalances() {
  const { data } = await supabase.from('wallet_balances').select('xont_balance, usdt_balance, tonx_balance').maybeSingle();
  return {
    xont: data ? Number(data.xont_balance) : 0,
    usdt: data ? Number(data.usdt_balance) : 0,
    tonx: data ? Number(data.tonx_balance) : 0,
  };
}
...
// 본인 노드 보상 기간 요약 — 보상은 구매일로부터 5년간 유지 (DAO 정책 6)
export async function getMyNodeRewardTerm() {
  const [{ data: all }, { data: eligible }] = await Promise.all([
    supabase.from('nodes').select('reward_expires_at'),
    supabase.from('reward_eligible_nodes').select('id'),
  ]);
  const dates = (all || []).map((n) => n.reward_expires_at).filter(Boolean).sort();
  return {
    total: (all || []).length,
    eligible: (eligible || []).length,
    firstExpiry: dates[0] || null,   // 가장 먼저 만료되는 노드
    lastExpiry: dates[dates.length - 1] || null,
  };
}
...
/* ---------------- 일자별 매출 · 노드풀 (revenue_daily) ---------------- */

export async function adminListRevenueDaily(limit = 60) {
  const { data, error } = await supabase.from('revenue_daily')
    .select('day, ad_revenue, subscription_revenue, token_revenue, node_pool_amount')
    .order('day', { ascending: false }).limit(limit);
  if (error) throw error; return data || [];
}

// 같은 날짜가 있으면 덮어쓰기(upsert). 이미 분배된 일자의 노드풀 변경은 DB 트리거가 차단.
export async function adminUpsertRevenueDaily(row) {
  const { data, error } = await supabase.from('revenue_daily')
    .upsert({
      day: row.day,
      ad_revenue: Number(row.ad_revenue) || 0,
      subscription_revenue: Number(row.subscription_revenue) || 0,
      token_revenue: Number(row.token_revenue) || 0,
      node_pool_amount: Number(row.node_pool_amount) || 0,
    }, { onConflict: 'day' })
    .select().single();
  if (error) throw error; return data;
}

export async function adminDeleteRevenueDaily(day) {
  const { error } = await supabase.from('revenue_daily').delete().eq('day', day);
  if (error) throw error; return true;
}

/* ---------------- 노드 보상 알고리즘 (운영수익 → TONX 분배) ----------------
   재원: revenue_daily.node_pool_amount(USD) · 환산: × krw_per_usd ÷ reference_price_krw
   분배: 해당 일자에 유효한 노드(활성 · 5년 미만) 수로 ÷N 균등
   시작: 노드세일즈 킥오프 + 6개월 이후 일자만 허용                          */

// 실행 전 미리보기 (쓰기 없음)
export async function adminPreviewNodeReward(day) {
  const { data, error } = await supabase.rpc('preview_node_rewards', { p_day: day });
  if (error) throw error;
  return (data && data[0]) || null;
}

// 실제 분배 실행 (같은 날짜 중복 실행 불가)
export async function adminDistributeNodeReward(day) {
  const { data, error } = await supabase.rpc('admin_distribute_node_rewards', { p_day: day });
  if (error) throw error;
  return (data && data[0]) || null;
}

// 분배 실행 이력
export async function adminListNodeRewardRuns() {
  const { data, error } = await supabase.from('node_reward_runs')
    .select('day, node_pool_usd, tonx_total, eligible_nodes, per_node_tonx, recipients, executed_at')
...
/* 추천수당 플랜(회원 표시용) — 지급 로직과 동일하게 revenue_allocations 를 읽는다.
   (예전엔 commission_levels 를 읽어, 배분 탭을 고쳐도 회원 화면이 안 따라오는 문제가 있었다) */
export async function getCommissionLevels() {
  const { data } = await supabase.from('revenue_allocations')
    .select('item_name, pct, active').eq('kind', 'commission');
  const pick = (name) => (data || []).find((r) => r.item_name === name && r.active !== false);
  return [
    { level: 1, label: '1대 추천', rate_pct: Number(pick('영업 1대')?.pct ?? 0), active: true },
    { level: 2, label: '2대 추천', rate_pct: Number(pick('영업 2대')?.pct ?? 0), active: true },
  ].filter((x) => x.rate_pct > 0);
}
...
// 노드 보상(배당) 내역 — 운영수익 분배분, TONX로 지급 (DAO 정책 2·4항)
export async function getMyNodeDividends() {
  const { data } = await supabase.from('tonx_transactions')
    .select('amount, counterparty, ref_day, created_at')
    .eq('kind', 'node_reward').eq('direction', 'in')
    .order('created_at', { ascending: false }).limit(200);
  return data || [];
}
...
/* 추천수당 레벨 CRUD 는 제거했다. 비율의 단일 출처는 revenue_allocations(노드판매 배분 탭)이며
   지급 함수(pay_referral_for_deposit · pay_center_head_for_deposit)가 그 값을 직접 읽는다. */
...
export async function adminOperatorDetail(id) {
  const [{ data: op }, { data: down }, { data: nodes }, { data: stakes }, { data: bal }, { data: tx }, { data: deps }, { data: utx }] = await Promise.all([
    ...
    supabase.from('nodes').select('id, status, price_usdt, purchased_at, reward_expires_at').eq('operator_id', id).order('purchased_at', { ascending: false }),
