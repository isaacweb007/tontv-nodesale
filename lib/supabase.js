...
// 추천수당 내역 — 실제 지급된 추천수당(xont_transactions kind='referral')
export async function adminListReferralCommissions() {
  const { data, error } = await supabase.from('xont_transactions')
    .select('amount, counterparty, created_at, node_operators(display_name, email)')
    .eq('kind', 'referral').eq('direction', 'in')
    .order('created_at', { ascending: false }).limit(500);
  if (error) throw error; return data || [];
}
...
export async function adminOperatorDetail(id) {
  const [{ data: op }, { data: down }, { data: nodes }, { data: stakes }, { data: bal }, { data: tx }, { data: deps }] = await Promise.all([
    ...
    supabase.from('staking').select('id, amount, months, apy, status, started_at, ends_at').eq('operator_id', id).order('started_at', { ascending: false }),
    supabase.from('wallet_balances').select('xont_balance, updated_at').eq('operator_id', id).maybeSingle(),
  ...
  const finance = {
    ...
    referralXont: sumKind('referral'),         // 추천수당 누적
  ...
  return { op, upline, downline: down || [], nodes: nodes || [], stakes: stk, deposits: deps || [], tx: txs, finance };
}
...
export async function adminListConfirmedSales() {
  const { data, error } = await supabase.from('deposits')
    .select('id, amount_usdt, quantity, tier, network, created_at, confirmed_at')
    .eq('status', 'confirmed')
    .order('created_at', { ascending: false })
    .limit(2000);
  if (error) throw error;
